...
rather than claiming identity continuity retroactively.
...
IPC v4 adds bounded resumable long polling over the existing durable run audit
...
An exact approval decision is committed with its run transition. A grant moves
the run back to running; a denial or an expired decision moves it to paused so
it cannot remain stranded in `awaiting_approval`. Expiry is persisted before a
deterministic error is returned, and replay of the same command returns the
same durable outcome.

The privileged-operation foundation uses a closed kind and typed non-secret
target, immutable authority/idempotency metadata, canonical request and payload
digests, and a separately identified record for every dispatch attempt. An
attempt persists its transport operation ID, exact wire digest, broker and
guest boot IDs, and absolute deadline before I/O. Request and result payloads
may later be pruned, but their tombstone digests, target, identity, attempts,
and reviews remain durable. See
[ADR 0003](../platform/adr/0003-durable-privileged-operation-journal.md).

The internal journal now includes a capability-focused application store and
coordinator, memory and SQLCipher adapters, exact idempotency replay, atomic
intent/payload and dispatch/attempt commits, validated rehydration, and a
bounded daemon startup recovery pass. Retry-safe interrupted attempts become
`retry_pending`; non-idempotent attempts become
`interrupted_needs_review`, and recovery performs no I/O. SQLCipher schema 11
also rejects zero broker/guest boot identities, non-positive attempt durations,
and transport identity reuse of the durable journal ID.

There is still no typed privileged execution gateway, proof-bearing daemon
session, public IPC for the journal, or production guest-control caller. The
Windows service keeps `guest_control` authorization-gated, capability
resolution keeps `guestGrant=false`, and Work remains fail-closed in Limited
Mode until the complete path and its Windows qualification are present.
